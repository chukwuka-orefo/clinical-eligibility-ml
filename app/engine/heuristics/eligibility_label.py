# app/engine/heuristics/eligibility_label.py
"""
...
def _load_admissions(path):
    ...
    if not path.exists():
        raise FileNotFoundError(
            "Admissions file not found at {0}".format(path)
        )
    ...
    if missing:
        raise ValueError(
            "Admissions table missing required columns: {0}".format(missing)
        )
...
def _load_stroke_phenotype(path):
    ...
    if not path.exists():
        raise FileNotFoundError(
            "Stroke phenotype file not found at {0}".format(path)
        )
    ...
    if missing:
        raise ValueError(
            "Stroke phenotype table missing required columns: {0}".format(missing)
        )
...
def _load_cardiovascular_phenotype(path):
    ...
    if not path.exists():
        raise FileNotFoundError(
            "Cardiovascular phenotype file not found at {0}".format(path)
        )
    ...
    if missing:
        raise ValueError(
            "Cardiovascular phenotype table missing required columns: {0}".format(missing)
        )
...
def _join_inputs(admissions, stroke, cvd):
    """
    ...
    df["has_any_cardiovascular_signal"] = (
        df["has_any_cardiovascular_signal"].fillna(False)
    )
    df["cardiovascular_code_count"] = (
        df["cardiovascular_code_count"].fillna(0).astype(int)
    )
